@push('scripts')
    ...
    <script>
        const myModal = document.getElementById('ImportModalCategory')
...
<x-app-admin-layout :assets="$assets ?? []">
    ...
    <x-modal modalId="ImportModalCategory" title="Import Categories" action="{{ route('admin.category.import') }}"
        method="POST" submitLabel="Import">
        <x-file name="file" id="file" label="Excel File" placeholder="Enter Excel File"
            accept="application/vnd.ms-excel, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            required="true" />

        <a href="{{ route('admin.category.template') }}"
            class="btn btn-primary d-flex align-items-center justify-content-center" data-bs-toggle="tooltip"
            data-bs-placement="top" title="Download the Excel template for category import">
